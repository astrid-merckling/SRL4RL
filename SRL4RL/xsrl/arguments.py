...
def get_args():
    parser = argparse.ArgumentParser(description='XSRL (eXploratory State Representation Learning)')
    parser.add_argument('--evalExplor', type=str2bool, default=False,
    ...
    parser.add_argument('--logs_dir', type=str, default='logsXSRL', help='path where to save the models')
    parser.add_argument('--method', type=str, default='XSRL', help='Method to use for training representations')
    ...
    parser.add_argument('--reset_policy', type=str2bool, default=False, help='only with dir: reset policy')    
    "Environment hyper-parameters"
    parser.add_argument('--env_name', type=str, default='TurtlebotMazeEnv-v0', choices=PYBULLET_ENV,help='Environment name')     
    parser.add_argument("--num_envs", type=int, default=32,
                        help="Number of envs to train in parallel, corresponds to the batch size")   
    parser.add_argument('--randomExplor', type=str2bool, default=False, help='Always use a constant reset state')
    ...
    "XSRL options"
    parser.add_argument('--resetPi', type=str2bool, default=False, help='resetPi trick for better diversity: train two discovery policies')
    parser.add_argument('--dvt_patience', type=int, default=2, help='Max counts before to resetPi')
    parser.add_argument('--weightInverse', type=float, default=0., help='weight')
    parser.add_argument('--weightLPB', type=float, default=0., help='weight')
    parser.add_argument('--weightEntropy', type=float, default=0., help='weight')
    parser.add_argument('--autoEntropyTuning', type=str2bool, default=False, help='Tune weightEntropy')
    ...
    parser.add_argument('--alpha_dim', type=int, default=30, help='alpha output dimension')
    ...
    parser.add_argument('--nb_hidden_gamma', type=str, default='128-512-128', help='number of hidden units per layer')
    parser.add_argument('--nb_hidden_pi', type=str, default='128-512-128', help='number of hidden units per layer')
    parser.add_argument('--cutoff', type=int, default=2, help='Number of layers in mu_tail and log_sig_tail')
    "SGD hyper-parameters"
    parser.add_argument('--pi_batchSize', type=int, default=128,
                        help='pi_batchSize for updating the discovery policy and inverse/LPB-target models')
    parser.add_argument('--pi_updateInt', type=int, default=512, help='maxStep for training env')    
    ...
    parser.add_argument('--lr_explor', type=float, default=1e-4, help='Learning rate for training')
    parser.add_argument('--lr_alpha', type=float, default=1e-4, help='Learning rate for training')
...
def assert_args_XSRL(args):
    ...
    if with_discoveryPi:
        ...
        assert args.pi_batchSize % args.nEnv_perPi == 0, \
        ...
        assert args.backprop_per_eval % args.pi_updateInt == 0, \
...
def giveXSRL_name(config):
    ...
    method_params += '[{}] '.format(config['noise_type']) if config['noise_type'] != 'none' else ''
    method_params += '[flickering-{}] '.format(config['flickering']) if config['flickering'] > 0 else ''
    method_params += 'dim {:02d}, maxStep {}, num envs {}, '.format(
        config['state_dim'],config['maxStep'],config['num_envs'])
    if with_discoveryPi:
        method_params += 'batchPi {}, pi_updateInt {}, '.format(config['pi_batchSize'], config['pi_updateInt'])
    method_params += 'dvt_patience-{}, '.format(config['dvt_patience']) if config['resetPi'] else ''
    method_params += 'wInverse-{}, '.format(config['weightInverse']) if config['weightInverse'] > 0 else ''
    method_params += 'wLPB-{}, '.format(config['weightLPB']) if config['weightLPB'] > 0 else ''
    method_params += 'optim-{}, '.format(config['optimizer']) if config['optimizer'] != 'adam' else ''
    method_params += 'hidden-{}, '.format(config['nb_hidden_gamma']) if config['nb_hidden_gamma'] != '128-512-128' else ''
    if with_discoveryPi:
        if config['autoEntropyTuning']:
            method_params += 'autoEnt, '
        else:
            method_params += 'wEnt-{}, '.format(config['weightEntropy']) if config['weightEntropy'] > 0 else ''

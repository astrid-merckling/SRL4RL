...
if args.maxStep == np.inf:
    ...
    maxSteps = np.linspace(
        args.maxStep, args.maxStep + args.num_envs - 1, args.num_envs
    ).astype(int)
...
if args.env_name in ["TurtlebotEnv-v0", "TurtlebotMazeEnv-v0"]:
    ...
    visuExplor_env = gym.make(
        ...
        randomExplor=False,
        wallDistractor=args.wallDistractor,
    ...
    visuExplor_env = gym.make(
        ...
        renders=False,
        maxSteps=maxSteps_visuExplor_env * args.actionRepeat,
    ...
    envs = [
        gym.make(
            ...
            env_name=args.env_name,
            renders=False,
            maxSteps=maxSteps[i_] * args.actionRepeat,
            actionRepeat=actionRepeat_env,
            image_size=image_size[-1],
            color=True,
            seed=args.seed + i_ + 1,
    ...
    envEval = gym.make(
        ...
        env_name=args.env_name,
        renders=False,
        maxSteps=maxStep_eval * args.actionRepeat,
        actionRepeat=actionRepeat_env,
        image_size=image_size[-1],
        color=True,
        seed=args.seed,
...
if args.keep_seed:
    ...
    createFolder(
        save_path + "xHat_nextObsEval", "xHat_nextObsEval folder already exist"
    )
...
if args.my_dir:
    ...
    beta = MLP_Module(
        ...
        hidden_beta + [args.beta_dim],
        activation=args.activation,
    )
    "Gamma function: predict next state"
    gamma = MLP_Module(
        args.beta_dim + args.alpha_dim,
        get_hidden(args.nb_hidden_gamma) + [args.state_dim],
        activation=args.activation,
    )
...
alpha.to(device).train(), beta.to(device).train(), gamma.to(device).train(), omega.to(
    device
).train()
...
if with_discoveryPi and reset_policy:
    ...
    pi_head, mu_tail, log_sig_tail, _ = MLP_mdn(
        args.state_dim,
...
if args.resetPi:
    ...
    pi_head_dvt, mu_tail_dvt, log_sig_tail_dvt, _ = MLP_mdn(
        args.state_dim,
        get_hidden(args.nb_hidden_pi) + [action_dim],
        cutoff=args.cutoff,
        activation=args.activation,
    )
    pi_head_dvt.to(cpu).train(), mu_tail_dvt.to(cpu).train(), log_sig_tail_dvt.to(
        cpu
    ).train()
    policyOptimizer_dvt = torch.optim.Adam(
        list(pi_head_dvt.parameters())
        + list(mu_tail_dvt.parameters())
        + list(log_sig_tail_dvt.parameters()),
        lr=args.lr_explor,
    )
else:
...
if with_discoveryPi:
    ...
    policyParameters = (
        list(pi_head.parameters())
        + list(mu_tail.parameters())
        + list(log_sig_tail.parameters())
    )

    if args.optimizer == "adam":
        policyOptimizer = torch.optim.Adam(policyParameters, lr=args.lr_explor)
    else:
        policyOptimizer = torch.optim.AdamW(
            policyParameters, lr=args.lr_explor, amsgrad=amsgrad
        )
    "force the Garbage Collector to release unreferenced memory"
    ...
    if args.inverse:
        ...
        if args.optimizer == "adam":
            ...
            iotaOptimizer = torch.optim.AdamW(
                list(iota.parameters()), lr=lr_inverse, amsgrad=amsgrad
            )
else:
...
early_stopper = EarlyStopping(
    ...
    name=args.method + "-{}".format(hashCode),
    min_delta=0.0001,
...
"Define losses"
Loss = (
    lambda x, y, den, reduction="none": torch.nn.MSELoss(reduction=reduction)(x, y)
    / den
)
Loss_obs = lambda x, y, den, reduction="none": torch.nn.MSELoss(reduction=reduction)(
    x, y
) / (den * config["n_stack"])
...
if args.autoEntropyTuning:
    ...
    log_temperatureTensor = numpy2pytorch(
        np.array(np.log(args.init_temperature)), differentiable=True, device=device
    )
...
if with_discoveryPi:
    ...
    DetLogCov_log, CovNorm_log, muNorm_log, LogPi_jacobian_log = [
        np.zeros((args.pi_backprop_per_eval), np.float32) for _ in range(4)
    ]
    lossInverse_log, LPB_log, entropy_log, temperature_log = [
        np.zeros((args.pi_backprop_per_eval), np.float32) for _ in range(4)
    ]
    lossInverseSequential = (
        r_entropySequential
    ) = r_inverseSequential = r_LPBsequential = 0
...
if args.resetPi:
    ...
    if args.weightInverse > 0 or args.weightLPB > 0:
        lossInverse_log_dvt, LPB_log_dvt = [
            np.zeros((args.pi_backprop_per_eval), np.float32) for _ in range(2)
        ]
...
stop_training = False
while (
    not early_stopper.early_stop
    and elapsed_epochs < args.n_epochs
    ...
    if with_discoveryPi:
        if args.resetPi:
            ...
            TensActions_dvt = torch.zeros(
                (args.nEnv_perPi, action_dim), dtype=torch.float32
            ).to("cpu")
            Tens_logPi_dvt = torch.zeros((args.nEnv_perPi, 1), dtype=torch.float32).to(
                "cpu"
            )
        TensActions = torch.zeros(
            (args.nEnv_perPi, action_dim), dtype=torch.float32
        ).to("cpu")
        Tens_logPi = torch.zeros((args.nEnv_perPi, 1), dtype=torch.float32).to("cpu")
        log_sig = torch.zeros((args.nEnv_perPi, action_dim), dtype=torch.float32).to(
            "cpu"
        )
        mu = torch.zeros((args.nEnv_perPi, action_dim), dtype=torch.float32).to("cpu")
        LogPi_jacobian = torch.zeros((args.nEnv_perPi, 1), dtype=torch.float32).to(
            "cpu"
        )
    else:
    ...
    while (True in has_bump) or (True in has_bump_dvt):
        ...
        if with_discoveryPi:
            ...
            if args.resetPi:
                (
                    TensActions[has_bump],
                    Tens_logPi[has_bump],
                    TensActions_dvt[has_bump_dvt],
                    Tens_logPi_dvt[has_bump_dvt],
                    log_sig[has_bump],
                    mu[has_bump],
                    LogPi_jacobian[has_bump],
                ) = policy_last_layer(
                    np2torch(states[: args.nEnv_perPi][has_bump]),
                    pi_head,
                    mu_tail,
                    log_sig_tail,
                    s_dvt=np2torch(states[args.nEnv_perPi :][has_bump_dvt]),
                    pi_head_dvt=pi_head_dvt,
                    mu_tail_dvt=mu_tail_dvt,
                    log_sig_tail_dvt=log_sig_tail_dvt,
                    config=config,
                    save_pi_logs=True,
                )
            ...
            ActEnv[has_bump] = np.random.uniform(
                low=-1, high=1, size=(sum(has_bump), action_dim)
            )
        if args.bumpDetection:
            has_bump = np.array(
                [
                    env_.bump_detection(a_)
                    for env_, a_ in zip(envs[: args.nEnv_perPi], ActEnv)
                ]
            )
            if args.resetPi:
                has_bump_dvt = np.array(
                    [
                        env_.bump_detection(a_)
                        for env_, a_ in zip(envs[args.nEnv_perPi :], ActEnv_dvt)
                    ]
                )
        else:
    ...
    if with_discoveryPi:
        if args.resetPi:
            ...
            ActEnv = pytorch2numpy(TensActions)
        DetLogCov_sum += pytorch2numpy(
            sumFct(2 * log_sig)
        )  # compute determinant of Log covariance
        CovNorm_sum += pytorch2numpy(
            meanFctNorm((2 * log_sig).exp())
        )  # compute norm of covariance
    ...
    if args.evalExplor:
        ...
        if True in endDetections:
            ...
            endDetection_dict[
                "endDetection"
            ] = "epochs {}, gradients {}, pi-gradients {}, steps {}".format(
    ...
    if args.resetPi:
        lossNextObs_dvt1_sum += pytorch2numpy(
            (
                Loss_obs(
                    xHat_next.to("cpu").detach()[: args.nEnv_perPi],
                    np2torch(next_observations[: args.nEnv_perPi]),
                    args.nEnv_perPi,
                ).sum((1, 2, 3))
            ).sum()
        )
        lossNextObs_dvt2_sum += pytorch2numpy(
            (
                Loss_obs(
                    xHat_next.to("cpu").detach()[args.nEnv_perPi :],
                    np2torch(next_observations[args.nEnv_perPi :]),
    ...
    if with_discoveryPi and (elapsed_steps % args.pi_sampling == 0):
        if args.inverse:
            ...
            lossInverseSequential += (
                Loss(actions_hat, np2torch(ActEnv), args.num_envs).sum(-1)
            ).sum()
            r_inverseSequential += (
                Loss(
                    actions_hat[: args.nEnv_perPi].detach(),
                    TensActions,
                    args.nEnv_perPi,
                ).sum(-1)
            ).sum()
        ...
        if args.entropy:
            ...
            if args.resetPi:
                r_entropySequential_dvt -= Tens_logPi_dvt.mean()
        else:
        ...
        if args.LPB:
            ...
            o_alphaLPB = (
                alpha(np2torchDev(observations[: args.nEnv_perPi])).to("cpu").detach()
            )
            if device == "cuda":
                beta.to("cpu"), gamma.to("cpu")
            o_betaLPB = beta(
                torch.cat((np2torch(states[: args.nEnv_perPi]), TensActions), dim=1)
            )  # keep only dependance on action parameters
            ...
            if device == "cuda":
                alpha_old.to(device)
            o_alpha_old = alpha_old(np2torchDev(observations[: args.nEnv_perPi])).to(
                "cpu"
            )
            o_beta_old = beta_old(
                torch.cat(
                    (np2torch(states[: args.nEnv_perPi]), TensActions.detach()), dim=1
                )
            )
            ...
            r_LPBsequential += (
                Loss(s_next_old, s_nextLPB, args.nEnv_perPi).sum(-1)
            ).sum()

            if args.resetPi:
                o_alphaLPB_dvt = (
                    alpha(np2torchDev(observations[args.nEnv_perPi :]))
                    .to("cpu")
                    .detach()
                )
                o_betaLPB_dvt = beta(
                    torch.cat(
                        (np2torch(states[args.nEnv_perPi :]), TensActions_dvt), dim=1
                    )
                )
                input_gammaLPB_dvt = torch.cat((o_alphaLPB_dvt, o_betaLPB_dvt), dim=1)
                s_nextLPB_dvt = gamma(input_gammaLPB_dvt)
                ""
                o_alpha_old_dvt = alpha_old(
                    np2torchDev(observations[args.nEnv_perPi :])
                ).to("cpu")
                o_beta_old_dvt = beta_old(
                    torch.cat(
                        (np2torch(states[args.nEnv_perPi :]), TensActions_dvt.detach()),
                        dim=1,
                    )
                )
                input_gamma_old_dvt = torch.cat(
                    (o_alpha_old_dvt, o_beta_old_dvt), dim=1
                )
                s_next_old_dvt = gamma_old(input_gamma_old_dvt)
                r_LPBsequential_dvt += (
                    Loss(s_next_old_dvt, s_nextLPB_dvt, args.nEnv_perPi).sum(-1)
                ).sum()
    ...
    if True in dones:
        ...
        obs = np.vstack(
            [envs[i_].reset() for i_ in np.arange(args.num_envs)[envs_to_reset]]
        )
        if config["n_stack"] > 1:
    ...
    if with_discoveryPi and (elapsed_steps % args.pi_updateInt == 0):
        ...
        if args.resetPi and not (args.weightInverse > 0 or args.weightLPB > 0):
            entropy_log_dvt[k_pi] = (
                pytorch2numpy(r_entropySequential_dvt) / args.nPi_samples
            )
        ...
        if args.inverse:
            ...
            lossInverse_log[k_pi] = pytorch2numpy(
                r_inverseSequential / args.nPi_samples
            )
            if args.resetPi:
                lossInverse_log_dvt[k_pi] = pytorch2numpy(
                    r_inverseSequential_dvt / args.nPi_samples
                )
        ...
        if args.autoEntropyTuning:
            ...
            temperature_loss = temperatureTensor * (
                r_entropySequential.detach() - args.nPi_samples * target_entropy
            )
        ...
        if args.LPB:
            ...
            if args.resetPi:
                LPB_log_dvt[k_pi] = pytorch2numpy(
                    r_LPBsequential_dvt / args.nPi_samples
                )
        ...
        lossPi = (
            -args.weightInverse * r_inverseSequential
            - args.weightLPB * r_LPBsequential
            - temperature * r_entropySequential
        )
        lossPi.backward()
        ...
        if args.resetPi:
            ...
            lossPi_dvt = (
                -args.weightInverse * r_inverseSequential_dvt
                - args.weightLPB * r_LPBsequential_dvt
                - temperature * r_entropySequential_dvt
            )
            lossPi_dvt.backward()
    ...
    if elapsed_gradients > 1 and (k_srl == args.backprop_per_eval) or stop_training:
        ...
        config["elapsed_epochs"] = elapsed_epochs = (
            elapsed_gradients // args.backprop_per_eval
        )
        config["elapsed_time_s"] = time() - start_time + loaded_time_s
        config["elapsed_time"] = elapsed_time = str(
            timedelta(seconds=(int(config["elapsed_time_s"])))
        )
        ...
        if args.resetPi:
            ...
            update_text(
                [lossNextObs_dvt1_mean], save_path + "lossNextObs_dvt1_mean.txt"
            )
            plotter(
                lossNextObs_dvt1_mean,
                save_path,
                name="lossNextObs_dvt1_mean",
                title="{} \n{}".format(SRL_name, hashCode),
                ylabel="Average next obs prediction errors",
                backprop_per_eval=args.backprop_per_eval,
                text_file=save_path + "lossNextObs_dvt1_mean.txt",
            )
            "dvt2"
            ...
            update_text(
                [lossNextObs_dvt2_mean], save_path + "lossNextObs_dvt2_mean.txt"
            )
            plotter(
                lossNextObs_dvt2_mean,
                save_path,
                name="lossNextObs_dvt2_mean",
                title="{} \n{}".format(SRL_name, hashCode),
                ylabel="Average next obs prediction errors",
                backprop_per_eval=args.backprop_per_eval,
                text_file=save_path + "lossNextObs_dvt2_mean.txt",
            )
            "compute mean intrinsic rewards of both policies"
            if args.weightInverse > 0 or args.weightLPB > 0:
                reward_dvt1_mean = args.weightInverse * np.mean(
                    lossInverse_log
                ) + args.weightLPB * np.mean(LPB_log)
                reward_dvt2_mean = args.weightInverse * np.mean(
                    lossInverse_log_dvt
                ) + args.weightLPB * np.mean(LPB_log_dvt)
            elif args.weightEntropy > 0:
            ...
            plotter(
                reward_dvt1_mean,
                save_path,
                name="reward_dvt1_mean",
                title="{} \n{}".format(SRL_name, hashCode),
                xlabel="policy gradient steps",
                ylabel="Average intrinsic reward 1st policy",
                backprop_per_eval=args.pi_backprop_per_eval,
                text_file=save_path + "reward_dvt1_mean.txt",
            )
            plotter(
                reward_dvt2_mean,
                save_path,
                name="reward_dvt2_mean",
                title="{} \n{}".format(SRL_name, hashCode),
                xlabel="policy gradient steps",
                ylabel="Average intrinsic reward 2nd policy",
                backprop_per_eval=args.pi_backprop_per_eval,
                text_file=save_path + "reward_dvt2_mean.txt",
            )
            print(
                " " * 100
                + "reward_dvt1_mean: {:.3f}           reward_dvt2_mean: {:.3f}\n".format(
                    reward_dvt1_mean, reward_dvt2_mean
                )
                + " " * 100
                + "lossNextObs_dvt1_mean: {:.3f}    lossNextObs_dvt2_mean: {:.3f}".format(
            ...
            if score_dvt2 > reward_dvt1_mean:
                ...
                print(
                    " " * 100 + f"dvt_counter: {dvt_counter} out of {args.dvt_patience}"
                )
                ...
                print(
                    " " * 100
                    + f"dvt_before2reset: {dvt_before2reset} out of {args.dvt_patience}"
                )
                ...
                print(
                    " " * 100
                    + f"dvtEquiv_before2reset: {dvtEquiv_before2reset} out of {args.dvt_patience}"
                )
            elif keep1stPolicy:
                dvt_counter = dvt_before2reset = 0
                print(" " * 100 + f"dvtEquiv and keep1stPolicy")
            if (
                (dvt_counter >= args.dvt_patience)
                or (dvt_before2reset >= args.dvt_patience)
                or (dvtEquiv_before2reset >= args.dvt_patience)
            ):
                if dvt_counter >= args.dvt_patience:
                    ...
                    "if the 2nd policy becomes better then better to follow the 2nd policy and reset the 1st policy" "which is equivalent to update the 1st policy with 2nd policy, and reset the 2nd policy"
                    keep1stPolicy = False
                    ...
                    update_text(
                        "[epoch {}] dvt_counter >= {}".format(
                            elapsed_epochs, args.dvt_patience
                        ),
                        save_path + "resetPi_log.txt",
                        text=True,
                    )
                    ...
                    print(
                        " " * 100
                        + f"~~ reset 2nd policy as it is not better than 1st one ~~"
                    )
                    keep1stPolicy_counter += 1
                    update_text(
                        "[epoch {}] dvt_before2reset >= {}".format(
                            elapsed_epochs, args.dvt_patience
                        ),
                        save_path + "resetPi_log.txt",
                        text=True,
                    ...
                    print(
                        " " * 100
                        + f"~~ reset 2nd policy as it is equivalent to 1st one ~~"
                    )
                    keep1stPolicy_counter = 0
                    update_text(
                        "[epoch {}] dvtEquiv_before2reset >= {}".format(
                            elapsed_epochs, args.dvt_patience
                        ),
                        save_path + "resetPi_log.txt",
                        text=True,
                ...
                if keep1stPolicy:
                    ...
                    log_sig_tail_dvt = update_target_network(
                        log_sig_tail_dvt, log_sig_tail
                    )
                else:
                ...
                pi_head_dvt.to(cpu).train(), mu_tail_dvt.to(
                    cpu
                ).train(), log_sig_tail_dvt.to(cpu).train()
                policyOptimizer_dvt = torch.optim.Adam(
                    list(pi_head_dvt.parameters())
                    + list(mu_tail_dvt.parameters())
                    + list(log_sig_tail_dvt.parameters()),
        ...
        if with_discoveryPi:
            ...
            plotter(
                ...
                title="{} \n{}".format(SRL_name, hashCode),
                xlabel="policy gradient steps",
                ylabel=r"Average $det(log (\Sigma))$",
                # avg over %s policy updates' % args.pi_backprop_per_eval,
                backprop_per_eval=args.pi_backprop_per_eval,
                text_file=save_path + "DetLogCov_mean.txt",
            )
            ...
            plotter(
                ...
                title="{} \n{}".format(SRL_name, hashCode),
                xlabel="policy gradient steps",
                ylabel=r"Average $\Vert\Sigma\Vert_2$",
                backprop_per_eval=args.pi_backprop_per_eval,
                text_file=save_path + "CovNorm_mean.txt",
            )
            ...
            plotter(
                ...
                title="{} \n{}".format(SRL_name, hashCode),
                xlabel="policy gradient steps",
                ylabel=r"Average $\Vert\mu_{\pi}(\mathbf{s}_{t})\Vert_2$",
                backprop_per_eval=args.pi_backprop_per_eval,
                text_file=save_path + "muNorm_mean.txt",
            )
            ...
            plotter(
                LogPi_jacobian_mean,
                save_path,
                name="LogPi_jacobian_mean",
                title="{} \n{}".format(SRL_name, hashCode),
                xlabel="policy gradient steps",
                ylabel=r"Average Jacobian of $log(\pi(\cdot\vert\mathbf{s}_{t})$)",
                backprop_per_eval=args.pi_backprop_per_eval,
                text_file=save_path + "LogPi_jacobian_mean.txt",
            )
            ...
            plotter(
                ...
                title="{} \n{}".format(SRL_name, hashCode),
                xlabel="policy gradient steps",
                ylabel=r"Average entropy of $\pi(\cdot \vert \mathbf{s}_{t})$",
                backprop_per_eval=args.pi_backprop_per_eval,
                text_file=save_path + "entropy_mean.txt",
            )

            print(
                "  DetLogCov: {:.3f}  entropy_mean: {:.3f}".format(
                    DetLogCov_mean, entropy_mean
                )
            )
        if args.inverse:
            ...
            plotter(
                lossInverse_mean,
                save_path,
                name="lossInverse_mean",
                title="{} \n{}".format(SRL_name, hashCode),
                xlabel="policy gradient steps",
                ylabel="Average inverse prediction errors",
                backprop_per_eval=args.pi_backprop_per_eval,
                text_file=save_path + "lossInverse_mean.txt",
            )
        if args.LPB:
            ...
            plotter(
                ...
                ylabel="Average LPB",
                backprop_per_eval=args.pi_backprop_per_eval,
                text_file=save_path + "LPB_mean.txt",
            )
        ...
        if args.autoEntropyTuning:
            ...
            plotter(
                ...
                backprop_per_eval=args.pi_backprop_per_eval,
                text_file=save_path + "temperature_mean.txt",
            )

        if args.evalExplor:
            update_text(
                robot_pos,
                save_path_robotPos + "robotPos_E{:06d}".format(elapsed_epochs),
                array=True,
            )
        ...
        plotter(
            ...
            name="nextObsEval",
            title="{} \n{}".format(SRL_name, hashCode),
            ylabel=r"Average next-obs prediction errors at test-time (after %s updates)"
            % args.backprop_per_eval,
            backprop_per_eval=args.backprop_per_eval,
            text_file=save_path + "nextObsEval_log.txt",
        )
        piExplore2obs(
            visuExplor_env,
            noise_adder,
        ...
        if with_discoveryPi:
            ...
            lossInverse_log, LPB_log, entropy_log, temperature_log = [
                np.zeros((args.pi_backprop_per_eval)) for _ in range(4)
            ]
        if args.resetPi:
            lossNextObs_dvt1_sum = lossNextObs_dvt2_sum = 0
            if args.weightInverse > 0 or args.weightLPB > 0:
        ...
        images_hat, images, im_next, im_high_render = (
            np.zeros([evalSteps, *image_size]),
            np.zeros([evalSteps, *image_size]),
            np.zeros([evalSteps, *image_size]),
            np.zeros([evalSteps, 256, 256, 3]),
        )
        for evalStep in range(evalSteps):
            with torch.no_grad():  # detach() not needed when torch.no_grad
                ...
                if with_discoveryPi:
                    ...
                    TensA = numpy2pytorch(
                        ActEnv, differentiable=False, device=device
                    ).unsqueeze(dim=0)
                ...
                for step_rep in range(actionRepeat):
                    ...
                    if config["n_stack"] > 1:
                        if (step_rep + 1) > (
                            config["actionRepeat"] - config["n_stack"]
                        ):
                            next_observation[
                                :, (step_rep - 1) * nc : ((step_rep - 1) + 1) * nc
                            ] = obs
                ...
                im_high_render[evalStep] = (
                    render_env(
                        envEval, 256, False, camera_id_eval, True, downscaling=False
                    )
                    / 255.0
                )
                "update inputs"
        ...
        plot_xHat(
            images[last_idx],
            images_hat[last_idx],
            imgTarget=im_next[last_idx],
            im_high_render=im_high_render[last_idx],
            imLabel=imLabel,
        ...
        for i in range(len(images)):
            plot_xHat(
                ...
                images_hat[i],
                imgTarget=im_next[i],
                im_high_render=im_high_render[i],
                imLabel=imLabel,
                figure_path=save_path + "xHat",
                with_noise=with_noise,
                with_nextObs=True,
                gradientStep=elapsed_gradients,

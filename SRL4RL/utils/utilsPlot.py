...
def plotter(loss_log, save_path, name='loss', title=None, xlabel='gradient steps', ylabel='Mini-Batch Loss',
            backprop_per_eval=None, text_file=None):
    ...
    if backprop_per_eval is None:
        ...
        x = np.arange(backprop_per_eval, (len(loss_log) + 1) * backprop_per_eval, backprop_per_eval)
    plt.plot(x, loss_log, '-k')
    plt.xlabel(xlabel)
    plt.ylabel(ylabel)
    if title:
        plt.title(title, fontsize=7, pad=2)
    plt.savefig(os.path.join(save_path, name + '.pdf'), format='pdf', bbox_inches='tight', pad_inches=0.1, )
    plt.close()

def visualizeMazeExplor(env_name, robot_pos=None, robot_pos_dir='', save_dir='', name=''):
    my_dpi = 150
    if robot_pos_dir:
    ...
    try:
        plt.savefig(os.path.join(save_dir, name + ".png"), bbox_inches='tight', pad_inches=0., format='png', dpi=my_dpi)
        plt.close()
    except:
...
def plot_xHat(img, img_hat, imgTarget=None, im_high_render=None, imLabel=None, figure_path='', name='xHat',
              gradientStep=None,saved_step=None, with_noise=False, with_nextObs=False, suffix='last', eval=False):
    im_size = 256
    color = img.shape[-1] == 3
    if img.shape[0] != im_size:
        ...
        img = cv2.resize(img, dsize=(im_size, im_size), interpolation=cv2.INTER_CUBIC).astype(np.float32) / 255.
    if img_hat.shape[0] != im_size:
        img_hat = (img_hat * 255).astype(np.uint8) if color else (img_hat * 255).astype(np.uint8)
        img_hat = cv2.resize(img_hat, dsize=(im_size, im_size), interpolation=cv2.INTER_CUBIC).astype(np.float32) / 255.
    if (imgTarget is not None) and (imgTarget.shape[0] != im_size):
        imgTarget = (imgTarget * 255).astype(np.uint8) if color else (imgTarget * 255).astype(np.uint8)
        imgTarget = cv2.resize(imgTarget, dsize=(im_size, im_size), interpolation=cv2.INTER_CUBIC).astype(
            np.float32) / 255.
    if (im_high_render is not None) and (im_high_render.shape[0] != im_size):
        im_high_render = (im_high_render * 255).astype(np.uint8) if color else (im_high_render * 255).astype(np.uint8)
        im_high_render = cv2.resize(im_high_render, dsize=(im_size, im_size), interpolation=cv2.INTER_CUBIC).astype(
            np.float32) / 255.
    ...
    fig, axs = plt.subplots(nrow, ncol, facecolor=(.88, .88, .88),
    ...
    if show_target:
        ...
        axId = 1
        axs[axId].axis('off')
        axs[axId].imshow(imgTarget) if color else axs[axId].imshow(np.dstack([imgTarget, imgTarget, imgTarget]))
        if with_nextObs:
    ...
    axs[axId].axis('off')
    axs[axId].imshow(img_hat) if color else axs[axId].imshow(np.dstack([img_hat, img_hat, img_hat]))
    ...
    # Create the arrow
    if show_target:
        axs[2].annotate('', xytext=(arrow_x, arrow_y), xy=(arrow_x+32, arrow_y), xycoords='figure points',
                        arrowprops=dict(arrowstyle="simple", color='k', ))
    else:
        axs[0].annotate('', xytext=(arrow_x, arrow_y), xy=(arrow_x+32, arrow_y), xycoords='figure points',
                        arrowprops=dict(arrowstyle="simple", color='k', ))
    ...
    plt.savefig(os.path.join(figure_path, name + epoch_ + ".png"), bbox_inches='tight', pad_inches=pad, format='png',
                dpi=my_dpi, facecolor=fig.get_facecolor(), edgecolor='none')
    plt.close()

    if eval:
        "save images separately"
        cv2.imwrite(os.path.join(figure_path, '{}_img.png'.format(name)), 255 * img[:, :, ::-1])
        cv2.imwrite(os.path.join(figure_path, '{}_img_hat.png'.format(name)), 255 * img_hat[:, :, ::-1])
        if imgTarget is not None:
            cv2.imwrite(os.path.join(figure_path, '{}_imgTarget.png'.format(name)), 255 * imgTarget[:, :, ::-1])
        if im_high_render is not None:
            cv2.imwrite(os.path.join(figure_path, '{}_{}.png'.format(name, imLabel)),
...
def angle2Dhsv(gt, normalizeAngle=False):
    ...
    if normalizeAngle:
        ...
        XminC = Xmin - (Xmax - Xmin) / 2  # in order to better visualize different colors
    else:
        XminC = Xmin - (Xmax - Xmin) / 10  # in order to better visualize different colors
...
def angle3Dhsv(gt, normalizeAngle=False):
    ...
    Xmax, Xmin, Ymax, Ymin = np.max(gt[:, 0]), np.min(gt[:, 0]), np.max(gt[:, 1]), np.min(gt[:, 1])
    if (np.mean(gt[:, 0]) - Xmin) / (Xmax - Xmin) < 0.5:
        gt[:, 0] = -gt[:, 0] + np.max(gt[:, 0])
        Xmax, Xmin = Xmax - Xmin, 0
    ...
    if normalizeAngle:
        ...
        angleMax, angleMin = np.max(gt[:, -1]), np.min(gt[:, -1])
        gt[:, -1] = 2 * np.pi * (gt[:, -1] - angleMin) / (angleMax - angleMin)
        YminC = Ymin - (Ymax - Ymin) / 2  # in order to better visualize different colors
        XminC = Xmin - (Xmax - Xmin) / 2  # in order to better visualize different colors
    else:
        YminC = Ymin - (Ymax - Ymin) / 2  # in order to better visualize different colors
        XminC = Xmin - (Xmax - Xmin) / 10  # in order to better visualize different colors
    # not needed because when modulo2PI was not done, angle is limited: gt[:, -1] = np.mod(gt[:, -1], (2 * np.pi))
    HSV = [(x[-1] * 1.0 / (2 * np.pi), (x[0] - XminC) / (Xmax - XminC), (x[1] - YminC) / max((Ymax - YminC), epsilon))
           for x in
           gt]
    RGB = [colorsys.hsv_to_rgb(*HSV_) for HSV_ in HSV]
...
def obs2torus(states):
    R = 1
    P=0.3
    states = np.vstack([(R+P* np.cos(states[:,1]))* np.cos(states[:,0]), (R+P* np.cos(states[:,1]))* np.sin(states[:,0]), P*np.sin(states[:,1])]).transpose(1,0)
    return states

def plotEmbedding(method, measures, all_states, figure_path='', gradientStep=None, saved_step=None, proj_dim=2,
                  n_neighbors=15, metric='euclidean', suffix='', env_name='TurtlebotMazeEnv-v0', eval=False):
    ...
    if env_name == 'TurtlebotMazeEnv-v0' and measures.shape[-1] == 2:
        ...
        gt4colors[:, 0] = gt4colors[:, 0] + (-2 * gt4colors[:, 0]) * (gt4colors[:, 1] < 1.04987811)
        colors = gt4colors[:, 0] + gt4colors[:, 1]
        gt2plot = measures
    elif env_name == 'TurtlebotMazeEnv-v0':
        withHSV = True
        proj_dim = 3
        measures[:, 0] = -measures[:, 0] + 1.5 * 4  # in order to better visualize the maze
    ...
    if method == 'UMAP':
        embedding = umap.UMAP(n_components=proj_dim, n_neighbors=n_neighbors, min_dist=min_dist,
                              metric=metric).fit_transform(all_states)
    elif method == 'PCA':
    ...
    if gt2plotParts is not None:
        ...
        if proj_dim == 3:
            ...
            proj_dimLast = 3
        if method == 'UMAP':
            embeddingParts['%s' % proj_dimLast] = (
                umap.UMAP(n_components=proj_dimLast, n_neighbors=n_neighbors, min_dist=min_dist,
                          metric=metric).fit_transform(all_states))
        elif method == 'PCA':
        ...
        for i, (gt2plot, colors, proj_dim) in enumerate(zip(gt2plotParts, colorsParts, proj_dimParts)):
            embedding = embeddingParts['%s' % proj_dim]
            if suffix != '':
                namePart = suffix + '-part%02d' % i
            else:
                namePart = 'part%02d' % i
            createFig(method, gt2plot, embedding, colors, cmap, GTlegend + ' robot part %01d' % i,
                      figure_path=figure_path, gradientStep=gradientStep, saved_step=saved_step, proj_dim=proj_dim,
                      suffix=namePart, eval=eval)
    else:
        createFig(method, gt2plot, embedding, colors, cmap, GTlegend, figure_path=figure_path,
                  gradientStep=gradientStep, saved_step=saved_step, proj_dim=proj_dim,
                  suffix=suffix, eval=eval)


def createFig(method, gt2plot, embedding, colors, cmap, GTlegend, figure_path='', gradientStep=None, saved_step=None,
              proj_dim=2, suffix='', eval=False):
    linewidth = 0
    size = 8
    "Create figure"
    if proj_dim == 2:
        ...
        axs[0].scatter(*gt2plot.T, s=size, c=colors, cmap=cmap, alpha=1.0, edgecolors='black', linewidth=linewidth)
        plt.setp(axs[0], xticks=[], yticks=[])
        axs[0].set_title(GTlegend)

        axs[1].scatter(*embedding.T, s=size, c=colors, cmap=cmap, alpha=1.0, edgecolors='black', linewidth=linewidth)
        ...
        if gt2plot.shape[-1] == 2:
            ...
            ax.scatter(*gt2plot.T, s=size, c=colors, cmap=cmap, edgecolors='black', linewidth=linewidth)
            plt.setp(ax, xticks=[], yticks=[])
            ax.set_title('Ground truth', y=1.05)
        else:
            ax = fig.add_subplot(1, 2, 1, projection='3d')
            ax.scatter(*gt2plot.T, s=size, c=colors, cmap=cmap, alpha=1.0, edgecolors='black', linewidth=linewidth)
        ...
        ax.scatter(*embedding.T, s=size, c=colors, cmap=cmap, alpha=1.0, edgecolors='black', linewidth=linewidth)
        plt.setp(ax, xticks=[], yticks=[], zticks=[])
    ...
    if saved_step is not None:
        ...
        plt.savefig(
            os.path.join(figure_path,
                         '{}proj/{}proj-dim{}{}'.format(method, method, proj_dim, suffix) + epoch_ + '.png'),
            bbox_inches='tight', format='png')  # pad_inches=0.1,
    # plt.savefig(path +".pdf", bbox_inches='tight', pad_inches=0.3, format='pdf')
    plt.savefig(os.path.join(figure_path, '{}proj-dim{}{}.pdf'.format(method, proj_dim, suffix)), bbox_inches='tight',

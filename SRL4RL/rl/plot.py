...
for curr_path in paths:
    ...
    if args.verbose:
        print(
            "\ncurr_path: {}, \nhashCode: {}, method: {}, model_name: {}".format(
                curr_path, config["hashCode"], config["method"], model_name
            )
        )
    ...
    if "gradient_steps" not in config:
        config["gradient_steps"] = config["n_batches"]
    if "eval_interval" not in config:
        config["eval_interval"] = 1
    backprop_per_eval = config["gradient_steps"] * config["eval_interval"]
    gradient_steps = np.arange(
        0, len(success_rate) * backprop_per_eval, backprop_per_eval
    )
    ...
    if args.smooth:
        ...
        if env_id in ["TurtlebotMazeEnv", "ReacherBulletEnv"]:
            ...
            x, y = moving_avg_curve(x, kernel_size=kernel_size), moving_avg_curve(
                y, kernel_size=kernel_size
            )
...
for env_id in sorted(data.keys()):
        ...
        for chosen_model in list_order:
            for nb_model, model_name in enumerate(data[env_id][option].keys()):
                if chosen_model in model_name:
                    ...
                    xs, ys = complete_pad(
                        xs, backprop_per_eval=backprop_per_eval_dict[env_id]
                    ), complete_pad(ys, last_value=True)
    ...
    plt.xlabel(
        "training steps {}".format(sctf_notation), fontsize="large", weight="bold"
    )

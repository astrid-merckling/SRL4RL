...
class StateWrapper(gym.Wrapper):
    ...
    def __init__(self, env, runner, demo, device=torch.device('cpu')):
        ...
        for step_rep in range(self.actionRepeat):
            ...
            if self.n_stack > 1 and self.runner.actionRepeat == 1:
                ...
                if (step_rep + 1) > (self.actionRepeat - self.n_stack):
                    obs[:, (step_rep - 1) * self.nc: ((step_rep - 1) + 1) * self.nc] = current_obs
...
class Runner():
    ...
    def forward(self, actor, x, eval=False, goal=None, random=False, env=None):
        if random:
            self.pi = env.action_space.sample()
        else:
            with torch.no_grad():
                self.pi = actor.select_actions(x, eval=eval, goal=goal)
        return self.pi
...
class StateRunner(Runner):
    def __init__(self, config):
        ...
        if 'SRL_name' in config: self.SRL_name = config['SRL_name']
        self.device = config['device']
        self.save_dir = config['save_dir']
        self.state_dim = config['state_dim']
        self.action_dim = config['action_dim']
        self.noise_type = config['noise_type']
        self.elapsed_gradients = self.elapsed_epochs = self.elapsed_time = self.elapsed_steps = 0
    ...
    def initState(self):
        return numpy2pytorch(np.random.normal(0, 0.02, self.state_dim), differentiable=False,
    ...
    def update_state(self, x, demo=False):
        with torch.no_grad():
            ...
            self.state = self.encoder(input.to(self.device))
        if demo:
            "save last input to record video"
...
class RandomNetworkRunner(StateRunner):
    def __init__(self, n_channels, config):
        ...
        if config['srl_path']:
            self.encoder = torch.load(os.path.join(config['srl_path'], 'state_model.pt'), map_location=torch.device('cpu'))
        elif config['dir']:
            self.encoder = torch.load(os.path.join(config['save_dir'], 'state_model.pt'), map_location=torch.device('cpu'))
        else:
            self.encoder = CNN(config['state_dim'], n_channels, activation=config['activation'], debug=config['debug'])
        self.encoder.eval()

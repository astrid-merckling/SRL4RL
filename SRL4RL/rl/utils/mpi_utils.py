...
def sync_params(parameters):
    ...
    # set the flat params back to the network
    pointer = 0
    getattr(parameters, attr).copy_(torch.tensor(flat_params[pointer:pointer + parameters.data.numel()]).view_as(parameters.data))
...
def sync_grads_single(parameters):
    ...
    # the pointer
    pointer = 0
    getattr(parameters, attr).copy_(torch.tensor(global_grads[pointer:pointer + parameters.data.numel()]).view_as(parameters.data))
    pointer += parameters.data.numel()
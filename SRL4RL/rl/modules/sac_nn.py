...
from SRL4RL.utils.nn_torch import import_weight_init, MLP_mdn, MLP_Module, pytorch2numpy
...
class QNetwork(nn.Module):
    def __init__(self, env_params, config):
        ...
        self.q1_network = MLP_Module(input_dim, nb_hidden, activation=activation, name='Q1')
        self.q2_network = MLP_Module(input_dim, nb_hidden, activation=activation, name='Q2')
...
class GaussianPolicy(nn.Module):
    def __init__(self, env_params, config):
        ...
        if config['cutoff'] > 0:
            self.pi_network, self.mean_linear, self.log_sig_linear, _ = MLP_mdn(input_dim,
                                                                                nb_hidden + [env_params['action']],
                                                                                cutoff=config['cutoff'],
                                                                                activation=config['activation'])
        else:
            self.pi_network = MLP_Module(input_dim, nb_hidden, activation=config['activation'],
                                  with_last_actv=with_last_actv, name='Pi')
        ...
        self.action_scale = torch.tensor((env_params['action_max'] - env_params['action_min']) / 2.)
        self.action_bias = torch.tensor((env_params['action_max'] + env_params['action_min']) / 2.)
    ...
    def normalizePi(self, mu, pi, log_pi):
        ...
        log_pi -= torch.log(self.action_scale * (1 - pi.pow(2)) + epsilon).sum(-1, keepdim=True)
    ...
    def select_actions(agent, state, eval=False, goal=None):
        if eval:
            _, _, pi = agent.sample(state, goal=goal)
        else:
            pi, _, _ = agent.sample(state, goal=goal)

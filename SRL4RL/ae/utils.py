...
def AE_nextObsEval(encoder, decoder,config, save_dir, gradientStep=None, saved_step=None, suffix='last', debug=False):
    ...
    video_out = cv2.VideoWriter(video_path, cv2.VideoWriter_fourcc(*'mp4v'), fps=fps,
                                frameSize=(int(588 * 2), 588)) if config['color'] else \
        cv2.VideoWriter(video_path, cv2.VideoWriter_fourcc(*'XVID'), fps=fps, frameSize=(int(588 * 2), 588),
                        isColor=0)
    ...
    loss_fn = lambda x, y: torch.nn.MSELoss(reduction='sum')(x, y) / (x.shape[0] * config['n_stack'])
    ...
    for step, obs in enumerate(observations):
        ...
        if (step + 1) % config['actionRepeat'] == 0:
            ...
            with torch.no_grad():
                ...
                "update loss"
                loss_log += pytorch2numpy(loss_fn(xHat, np2torch(observation, device).unsqueeze(0)))

            "update video"
            update_video(im=255 * CWH2WHC(observation[-3:, :, :]), color=config['color'], video_size=588,
                         video=video_out, fpv=config['fpv'],
            ...
            if saveIm:
                ...
                if config['n_stack'] > 1:
                    ...
                    for step_r in range(config['n_stack']):
                        ...
                        plot_xHat(CWH2WHC(observation[step_r * nc: (step_r + 1) * nc]),
                                  tensor2image(xHat[:, step_r * nc: (step_r + 1) * nc]),
                                  figure_path=save_dir,
                                  with_nextObs=False, name=name, gradientStep=gradientStep,
                                  suffix=suffix, eval=eval)
                else:
                    plot_xHat(CWH2WHC(observation[-3:, :, :]), tensor2image(xHat[:, -3:, :, :]),
                              figure_path=save_dir,
                              with_nextObs=False, name=name, gradientStep=gradientStep,
                              suffix=suffix, eval=eval)
                if elapsed_steps == xHat_nextObsEval_step:
                    if saved_step is not None:
                        plot_xHat(CWH2WHC(observation[-3:, :, :]), tensor2image(xHat[:, -3:, :, :]),
                                  ...
                                  with_nextObs=False, name='xHat_nextObsEval',
...
class AERunner(StateRunner):
    def __init__(self, config):
        ...
        if srl_path:
            print('Load: encoder')
            self.encoder = torch.load(os.path.join(srl_path, 'state_model.pt'),

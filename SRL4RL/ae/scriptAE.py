...
if args.env_name in ["TurtlebotEnv-v0", "TurtlebotMazeEnv-v0"]:
    ...
    envs = [
        gym.make(
            ...
            randomExplor=args.randomExplor,
...
decoderOptimizer = torch.optim.Adam(
    parametersDecoder, lr=args.lr, weight_decay=args.decoder_weight_lambda
)
"force the Garbage Collector to release unreferenced memory"
...
# Policy configs
with_policy = args.randomExplor and args.env_name in [
...
if with_policy:
    ...
    o_mean, o_std, g_mean, g_std, actor = loadPi(
        PiTrained_path, model_type="model_best"
    )
...
while not early_stopper.early_stop and elapsed_epochs < args.n_epochs:
    ...
    while True in has_bump:
        if with_policy and np.random.uniform(0, 1) < 0.5:
            ...
            x = [
                process_inputs_gt(state_, None, o_mean, o_std, g_mean, g_std)
        ...
        if args.bumpDetection:
            has_bump = np.array(
                [
                    ...
                    for env_, a_ in zip(envs[: args.num_envs], ActEnv)
                ]
            )
        else:
    ...
    for step_rep in range(actionRepeat):
        ...
        if config["n_stack"] > 1 and args.actionRepeat == 1:
            ...
            if (step_rep + 1) > (actionRepeat - config["n_stack"]):
                ...
                ] = add_noise(np.vstack(obs), noise_adder, config)
                targets[:, (step_rep - 1) * nc : ((step_rep - 1) + 1) * nc] = np.vstack(
                    obs
                )
    ...
    if True in dones:
        ...
        if envs_to_reset[0] and save_video:
            ...
            video_out = cv2.VideoWriter(
                os.path.join(
                    save_path, "{}_E{:03d}.mp4".format(args.new_env_name, n_video)
                ),
                cv2.VideoWriter_fourcc(*"mp4v"),
                fps=5,
                frameSize=(args.image_size, args.image_size),
    ...
    if elapsed_gradients > 1 and (k_srl == args.backprop_per_eval):
        ...
        plotter(
            ...
            text_file=save_path + "lossAE_mean.txt",
        )
        ...
        if not args.debug:
            ...
            plotter(
                ...
                name="nextObsEval",
                title="{} \n{}".format(SRL_name, hashCode),
                ylabel=r"Average obs prediction errors at test-time (after %s updates)"
        ...
        images_hat, images, im_target, im_high_render = (
                ...
                for step_rep in range(actionRepeat):
                    ...
                    if config["n_stack"] > 1 and args.actionRepeat == 1:
                        ...
                        if (step_rep + 1) > (actionRepeat - config["n_stack"]):
                            observation[
                                :, (step_rep - 1) * nc : ((step_rep - 1) + 1) * nc
                            ] = add_noise(obs, noise_adder, config)
                            target[
                                :, (step_rep - 1) * nc : ((step_rep - 1) + 1) * nc
                            ] = obs
                    elif (step_rep + 1) == actionRepeat:
